...
public class LongMethod extends ViewPart {
		...
		public void selectionChanged(IWorkbenchPart sourcepart, ISelection selection) {
			if (selection instanceof IStructuredSelection) {
				...
				if(javaProject != null && !javaProject.equals(selectedProject)) {
					...
					System.out.println("CHECKING = " + javaProject.toString());
					...
					System.out.println("GROUP LENGTH = " + sliceGroupTable.length);
					
					//for(int i=0;i<sliceGroupTable.length;i++)
					//{
					int i=2;
					
						Set<ASTSlice> testSlices = sliceGroupTable[i].getCandidates();
						System.out.println(i + ": Candidates size = " + sliceGroupTable[i].getCandidates().size() + "\n");
						
						int sliceCounter = 0;
						for(ASTSlice s : testSlices)
						{						
							System.out.println(sliceCounter+1 + " - Slice: Variable = " + s.getLocalVariableCriterion().getName().toString());
							
							System.out.println("------------ sliceToString ------------ \n" + s.sliceToString());
							System.out.println("------------ toString ------------ \n" + s.toString() + "\n");
							
							Object[] highlighted = s.getHighlightPositions();
							//Set<Position> pos = (LinkedHashMap) highlighted[0].keySet();
							
							System.out.println("------------ annotationMap ------------ \n" + highlighted[0].toString() + "\n");
							System.out.println("------------ duplicationMap ------------ \n" + highlighted[1].toString() + "\n");
							
							sliceCounter++;
						}
						
						//sliceGroupTable[i].getLocalVariableCriterion().getName().toString();
	...
	private void makeActions() {
		...
		identifyBadSmellsAction.setToolTipText("Identify Bad Smells Fanis");
	...
	public static void processMethod(final List<ASTSliceGroup> extractedSliceGroups, ClassObject classObject, AbstractMethodDeclaration methodObject) {
